...
namespace Cuahangchamsocthucung.Migrator
{
    public class MultiTenantMigrateExecuter : ITransientDependency
    {
        ...
        {
            var hostConnStr = CensorConnectionString(
                _connectionStringResolver.GetNameOrConnectionString(
                    new ConnectionStringResolveArgs(
                        MultiTenancySides.Host)));

            if (hostConnStr.IsNullOrWhiteSpace())
            {
                _log.Write(
                    "Configuration file should contain a connection string named 'Default'");
            ...
            {
                _log.Write(
                    "Continue to migration for this host database and all tenants..? (Y/N): ");
            ...
            {
                // Seed Host database
                // SeedHelper.SeedHostDb hiện nhận thêm IIocResolver
                _migrator.CreateOrMigrateForHost(
                    context =>
                        SeedHelper.SeedHostDb(
                            context,
                            _iocResolver));
            }
            catch (Exception ex)
            ...
            for (var i = 0; i < tenants.Count; i++)
            {
                ...
                _log.Write(
                    string.Format(
                        "Tenant database migration started... ({0} / {1})",
                        i + 1,
                        tenants.Count));
                ...
                if (!migratedDatabases.Contains(
                    tenant.ConnectionString))
                {
                    try
                    {
                        _migrator.CreateOrMigrateForTenant(
                            tenant);
                    }
                    catch (Exception ex)
                    {
                        ...
                        _log.Write(
                            "Skipped this tenant and will continue for others...");
                    }

                    migratedDatabases.Add(
                        tenant.ConnectionString);
                }
                else
                {
                    _log.Write(
                        "This database has already migrated before " +
                        "(you have more than one tenant in same database). " +
                        "Skipping it....");
                }

                _log.Write(
                    string.Format(
                        "Tenant database migration completed. ({0} / {1})",
                        i + 1,
                        tenants.Count));
        ...
        private static string CensorConnectionString(
