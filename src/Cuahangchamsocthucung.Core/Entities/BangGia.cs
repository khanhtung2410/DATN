...
namespace Cuahangchamsocthucung.Entities
{
    public class BangGia : AuditedEntity<int>, IValidatableObject
    {
        ...
        [Required]
        [Range(0, int.MaxValue, ErrorMessage = "Khoảng cân nặng không hợp lệ.")]
        ...
        [Required]
        [Range(typeof(decimal), "0.01", "79228162514264337593543950335", ErrorMessage = "Giá dịch vụ không hợp lệ.")]
        ...
        public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
        {
            if (Cannangtu>= Cannangden)
            {
                yield return new ValidationResult(
                    ...
                    new[] { nameof(Cannangtu), nameof(Cannangden) });
            }
        }
        public BangGia()
        {
        }
        public BangGia(int dichvuId, decimal giadv, string loaithucung, int cannangtu, int cannangden, bool loailong)
        {
            DichVuId = dichvuId;
            Giadv = giadv;
